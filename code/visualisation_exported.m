classdef visualisation_exported < matlab.ui.componentcontainer.ComponentContainer
    ...
    properties (Access = private, Transient, NonCopyable)
        CyclesPerSecondDropDown       matlab.ui.control.DropDown
    ...
    methods (Access = private)
        ...
        % Value changed function: CyclesPerSecondDropDown
        function CyclesPerSecondDropDownValueChanged(comp, event)
            value = comp.CyclesPerSecondDropDown.Value;
            f = 50; % 50Hz
            T = 1/f;
            if comp.Running
                resetSys(comp)
                msgbox('Time step was changed. Resetting Simulation')
            end
            comp.TimeStep = T/str2double(value);
            comp.t = 0:comp.TimeStep:comp.Duration;
        end
    end
    ...
    methods (Access = protected)
        ...
        function setup(comp)
            ...
            % Create CyclesPerSecondDropDownLabel
            comp.CyclesPerSecondDropDownLabel = uilabel(comp);
            comp.CyclesPerSecondDropDownLabel.HorizontalAlignment = 'right';
            comp.CyclesPerSecondDropDownLabel.Position = [298 757 107 22];
            comp.CyclesPerSecondDropDownLabel.Text = 'Cycles Per Second';

            % Create CyclesPerSecondDropDown
            comp.CyclesPerSecondDropDown = uidropdown(comp);
