...
if (x.e3 < x.Vz)       
    ...
    id = ((x.V2(3)- x.e2-k3)/2 - x.Vth) / x.Rb;
    k4 = [h/x.C * ( (id>=0)*id - ( (x.e2 + k3(1)/2) * (x.Rz*x.RL1) - x.Vz*x.RL1 + (x.iL + k3(2)/2)*x.Rz*x.RL1 )/Rd );
          h/x.L * ( ( ((x.e2 + k3(1)/2)*x.Rz*x.RL1 + x.Vz*x.RL1*x.Rs + (x.iL + k3(2)/2)*x.Rs*x.Rz) / Rd ) - x.RL2*(x.iL + k3(2)/2) )];
end
